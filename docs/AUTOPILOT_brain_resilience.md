...
- [x] `state.ts`: recognize current idle footers (`bypass permissions on`, `N shell(s)`,
      `← for agents`, `↓ to manage`, completed-spinner `✻ <Verb>ed for Xm Ys`) as READY;
      background shells do NOT block turn-end.
- [x] Feedback survey: dismiss with `0` (fall back to Esc), not Esc alone.
- [x] Verified submission: bracketed-paste the prompt, verify work actually started,
      re-send Enter (bounded) if the input was swallowed. Multi-line prompts safe.
- [x] Exit diagnostics: on claude exit, error includes exit code + last screen lines;
      claude version logged at spawn.
- [x] Unit tests for all new classifications + submit logic where testable offline.

### Phase 2 — recovery ladder (commit 2)
- [x] Transcript ground truth (`reader.ts`): file activity (mtime/size) = progress;
      last-entry-is-final-assistant-text + quiet = turn over.
- [x] `waitForReady` ladder instead of fail-fast: transcript check → repaint nudge
      (PTY resize) → (Phase 3: Qwen triage) → recoverable timeout.
- [x] Orchestrator turn recovery: on recoverable timeout / claude exit, kill + respawn
...
- [x] Supervisor auto-heal: run ends `error` → auto-continue with backoff + attempt cap
      (skip Auth/Cwd errors); notify only when exhausted. Config knob in `reliability`.
- [x] Tests: transcript liveness parsing, recovery decision logic.

### Phase 3 — leverage Qwen (commit 3)
- [x] `brain/triage.ts`: Qwen classifies an unrecognized/frozen screen tail
...
- [x] Brain JSON self-repair: one corrective retry before the fail-safe stop.
- [x] Escalation timeout for `autonomous`-persona sessions: auto-pick the first option
      after N min (config `brain.escalationTimeoutMin`, default 20; 0 = off) + notify.
- [x] Tests with stubbed LLM.

### Phase 4 — headless engine (commit 4)
- [x] `session/headlessSession.ts`: drives `claude -p --output-format stream-json`
      per turn with `--session-id`/`--resume` chaining (subscription-auth, no API key,
      no PTY, no screen scraping). Prompt via stdin.
- [x] Engine dispatch: `SessionEngine` gains `"claude-headless"`; orchestrator accepts a
      session factory; supervisor routes the new engine through it.
- [x] Tests: stream-json parsing (offline fixtures).
